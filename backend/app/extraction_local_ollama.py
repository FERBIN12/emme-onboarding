"""Local-only extraction path, NOT used in the primary flow.

Kept for reference: this is the local Ollama vision approach we tried
before falling back to Gemini (app/extraction.py) as primary. See that
module's docstring for why. This file is not imported by the running
app; test_extraction_local_ollama.py exercises it standalone if you want
to revisit local-only inference on different hardware.

Contract, same as the primary path:
